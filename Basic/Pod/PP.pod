...
Set to C code which declares additional variables to be added to the trans
structure. Code may include PP macros (e.g. C<< $COMP >>).
...
Allocates space for C<< $COMP >> items which require it.
See L<PDL::PP::Rule::MakeComp>.
